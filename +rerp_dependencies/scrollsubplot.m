function theAxis = scrollsubplot(nrows, ncols, thisPlot, fig)
...
elseif(isempty(position))
  ...
  if (min(thisPlot) < 1)&0
    ...
    PERC_OFFSET_L = 0.07;
    PERC_OFFSET_R = 0.085;
    PERC_OFFSET_B = .9*PERC_OFFSET_L;
    PERC_OFFSET_T = 5*PERC_OFFSET_R;
    if nrows > 2
      PERC_OFFSET_T = .9*PERC_OFFSET_T;
      PERC_OFFSET_B = .9*PERC_OFFSET_B;
    end
    if ncols > 2
    ...
    if ~strcmp(get(fig,'defaultaxesunits'),'normalized')
      ...
      def_pos = get(fig,'DefaultAxesPosition')+[-.05 -.05 +.1 +.05];
    end
...
if(kill_siblings)
  if delay_destroy
    if nargout
      ...
      set(fig,'NextPlot','replace'); return,
    end
  end
...
if isempty(scroll_clbk_indx)
  uicontrol('Units','normalized',...
            ...
            'Value',1,...
            'visible','off',...
            'Callback', 'rerp_dependencies.scroll(1)');
end
...
rerp_dependencies.scroll(1, fig)
